 ...
 * The add-car form.
 *
 * Mounted only while open — see AddCarButton — so a reopened sheet is a fresh
 * one and there is nothing to reset. That is deliberate: a half-filled draft
 * restored days later describes a car the person may no longer be adding.
 *
...
const AddCarSheet = ({
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    const parsed = carSchema.safeParse(candidate)
    ...
    if (result.status === 'success') {
      ...
      onOpenChange(false)
      // Re-reads the garage on the server, so the new car appears without
      // a full page load and without this component owning the list.
      router.refresh()
      return
    }
    ...
    >
      <Dialog.Portal>
        {/* Base UI holds the popup mounted until these transitions finish, and
            ...
            motion-safe only: someone who has asked for reduced motion gets the
            sheet with no transition to wait on, so it appears at once. */}
        <Dialog.Backdrop
          className={cn(
        ...
        <Dialog.Popup
          className={cn(
            ...
            // Off-screen at both ends, so the same class describes the slide
            // in and the slide out.
          ...
          <div className="flex items-start justify-between gap-4 border-b border-primary px-6 py-5">
            <div>
              ...
              <Dialog.Description className="mt-1 flex items-center gap-2 text-sm text-text-secondary">
                {/* Rex's eyes, the mark from the design. The Rex component itself is
                                    the launcher disc — glow, pulse and all — which is wrong at 16px. */}
                <svg
                  viewBox="0 0 28 20"
            ...
            <div className="flex-1 space-y-5 overflow-y-auto px-6 py-6">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Input
                    label="Year"
                    // Not type="number": the spinners and scroll-to-change
                    // are a liability on a four-digit field, and iOS still
                    // gets the numeric keypad from inputMode.
                    inputMode="numeric"
                    autoComplete="off"
                  ...
                  {USAGE_TYPES.map((usage) => {
                    ...
                    return (
                      <button
                        key={usage.value}
                        type="button"
                        // Radio semantics, not six unrelated buttons:
                        // one of these is chosen, and a screen reader
                        // should say which.
                        role="radio"
              ...
              <div>
                ...
                <textarea
                  ...
                  // counter cannot promise room the API refuses.
                  maxLength={NOTES_MAX}
