...
describe('Input labelling', () => {
  ...
  it('announces helper text the same way', () => {
    render(<Input label="Engine" helperText="This helps Rex give engine-specific advice" />)

    expect(screen.getByLabelText(/ENGINE/i)).toHaveAccessibleDescription(
      'This helps Rex give engine-specific advice'
    )
  })

  it('lets a caller supply its own id', () => {
    render(<Input label="Make" id="chosen-id" />)

    expect(screen.getByLabelText(/MAKE/i)).toHaveAttribute('id', 'chosen-id')
  })
