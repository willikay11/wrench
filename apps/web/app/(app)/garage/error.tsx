 ...
 *
 * unstable_retry re-runs the fetch, where reset would only re-render the same
 * failure — the distinction Next 16 draws between the two.
 */
export default function GarageError({
  ...
  return (
    ...
    >
      ...
      {/* No mention of the cars being gone: nothing here knows that, and
                the failure is on our side of the wire. */}
      ...
      {/* The digest is the only handle support has on a specific failure,
                and it is meaningless to an attacker. The message itself is not
                shown: it can carry internals a user should never read. */}
      {error.digest ? (
        <p className="mt-6 text-xs text-text-faint">Reference: {error.digest}</p>
      ) : null}
    </div>
  )
}
